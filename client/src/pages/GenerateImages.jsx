...
const GenerateImages = () => {
  const imageStyle = [
    'Realistic', 'Ghibli style', '3D style', 'Portrait style', 'Cartoon style', 'Anime style', 'Fantasy style', 'Realistic style'
  ]
  ...
  return (
    <div className='h-full overflow-y-scroll p-6 flex items-start flex-wrao gap-4 text-slate-700'>
      ...
      <form onSubmit={onSubmitHandler} className='w-full max-w-lg p-4 bg-white border border-gray-200 rounded-lg'>
        ...
        <textarea onChange={(e) => setInput(e.target.value)} value={input} rows={4} className='w-full mt-2 p-2 px-3 border border-gray-300 rounded-md outline-none text-sm' placeholder='Describe what you want to see in the image ...' required />
        ...
        <button disabled={loading} className='w-full flex items-center justify-center gap-2 mt-6  text-white px-4 py-2 rounded-lg bg-gradient-to-r from-[#00AD25] to-[#04FF50] text-sm cursor-pointer'>
          {
